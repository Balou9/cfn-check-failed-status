...
then
  ...
  bucket_list_abt_delete=$(
    aws cloudformation describe-stack-events \
      --stack-name=$STACK_NAME \
      | jq -r '.StackEvents[] | select(.ResourceType == "AWS::S3::Bucket") | select(.ResourceStatus | startswith("CREATE")) | select(.ResourceStatusReason != null) | select(.ResourceStatusReason=="Resource creation Initiated" or (.ResourceStatusReason | test("already exists"))) | .PhysicalResourceId'
  )
